...
public class WasteActivity extends AppCompatActivity {
        ...
        button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                else
                {
                    ...
                    title="Calulation Results";

                    //因為co2是float型態，所以必須cast成int
                    if(co2>=1)
                    {
                        notify = "Co-generation of waste disposal " + (int)co2 +" kg carbon footprint";
                        saveData(co2*1000);
                    }
                    else if(((int)(co2*100))!=0) //大於0.01kg的
                    {
                        notify = "Co-generation of waste disposal " + (int)(co2*1000) +" g carbon footprint";
                        saveData(co2*1000);
                    }
                    else
                    {
    ...
    public void saveData(float co2)
    {
        ...
        if(waste_category==R.id.incineration)
        {
            ...
            id = dataHelper.append(db, (int)co2,item_name+"Incinerator");
        }
        else if(waste_category==R.id.waste_water)
        {
            id = dataHelper.append(db, (int)co2,"Waste Water");
        }
        else//recover
        {
            id = dataHelper.append(db, (int)co2,"Recyle");
        }
